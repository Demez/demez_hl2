...
//-----------------------------------------------------------------------------
void PropPortalEffectCallback( const CEffectData &data )
{
	// CSmartPtr<CNewParticleEffect> pEffect = CNewParticleEffect::Create( data.GetEntity(), "portal_1_badsurface" );
}

DECLARE_CLIENT_EFFECT_BEGIN( portal_1_badsurfaceee, PropPortalEffectCallback )
// PRECACHE_REGISTER_BEGIN( GLOBAL, FX_PropPortal )

	PRECACHE( PARTICLE_SYSTEM, "portal_1_particles" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_particles" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_edge" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_edge" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_nofit" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_nofit" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_overlap" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_overlap" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_badvolume" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_badvolume" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_badsurface" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_badsurface" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_close" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_close" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_cleanser" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_cleanser" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_near" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_near" )
	PRECACHE( PARTICLE_SYSTEM, "portal_1_success" )
	PRECACHE( PARTICLE_SYSTEM, "portal_2_success" )
...
void C_Prop_Portal::Precache( void )
{
	// BaseClass::Precache();
	...
	// probably should move this to global scope
...
#if ENGINE_NEW
	PrecacheEffect( "portal_1_particles" );
	PrecacheEffect( "portal_2_particles" );
	PrecacheEffect( "portal_1_edge" );
	PrecacheEffect( "portal_2_edge" );
	PrecacheEffect( "portal_1_nofit" );
	PrecacheEffect( "portal_2_nofit" );
	PrecacheEffect( "portal_1_overlap" );
	PrecacheEffect( "portal_2_overlap" );
	PrecacheEffect( "portal_1_badvolume" );
	PrecacheEffect( "portal_2_badvolume" );
	PrecacheEffect( "portal_1_badsurface" );
	PrecacheEffect( "portal_2_badsurface" );
	PrecacheEffect( "portal_1_close" );
	PrecacheEffect( "portal_2_close" );
	PrecacheEffect( "portal_1_cleanser" );
	PrecacheEffect( "portal_2_cleanser" );
	PrecacheEffect( "portal_1_near" );
	PrecacheEffect( "portal_2_near" );
	PrecacheEffect( "portal_1_success" );
	PrecacheEffect( "portal_2_success" );
#endif
}
...
void C_Prop_Portal::Simulate()
{
...
#if ENGINE_NEW
		return ret;  // DEMEZ PORTAL: maybe false?
#else
		return;
#endif
	}
	...
	//now, fix up our list of ghosted renderables.
	{
		...
		for( int i = m_hGhostingEntities.Count(); --i >= 0; )
		{
			...
			{
				if( pRenderable == m_GhostRenderables[j]->m_hGhostedRenderable.Get() )
				{
					bStillInUse[j] = true;
...
int C_Prop_Portal::DrawModel( int flags RENDER_INSTANCE_INPUT )
{
	...
	// iRetVal = BaseClass::DrawModel( flags RENDER_INSTANCE );
