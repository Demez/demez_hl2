...
PRECACHE_MATERIAL( "shadertest/wireframe" )
PRECACHE_MATERIAL( "engine/writez_model" )
PRECACHE_MATERIAL( "engine/TranslucentVertexColor" )
...
void CPortalRender::WaterRenderingHandler_PreReflection() const
{
	...
	{
		CMatRenderContextPtr pRenderContext( materials );
		SET_STENCIL_ENABLE( pRenderContext, false );
	}
}

// DEMEZ PORTAL: uhh this might not be right lol
void CPortalRender::WaterRenderingHandler_PostReflection() const
{
	if( (m_iViewRecursionLevel > 0) && ShouldUseStencilsToRenderPortals() )
	{
		CMatRenderContextPtr pRenderContext( materials );
		SET_STENCIL_ENABLE( pRenderContext, true );
	}
}
...
void CPortalRender::EnteredPortal( CPortalRenderable *pEnteredPortal )
{
	...
	if ( cl_useOldSwapPortalVisibilityCode.GetInt() )
	{
		...
		PortalViewIDNode_t *pExitPortalsNewNode = AllocPortalViewIDNode( iNodeLinkCount );
		{
			...
			PixelVisibility_ShiftVisibilityViews( SS_SLOT_ARG() VIEW_MAIN, pExitPortalsNewNode->iPrimaryViewID );
			PixelVisibility_ShiftVisibilityViews( SS_SLOT_ARG() VIEW_3DSKY, pExitPortalsNewNode->iPrimaryViewID + 1 );
		}
		...
		if( pNewHead ) //it's possible we entered a portal we couldn't see through
		{
			...
			PixelVisibility_ShiftVisibilityViews( SS_SLOT_ARG() pNewHead->iPrimaryViewID, VIEW_MAIN );
			PixelVisibility_ShiftVisibilityViews( SS_SLOT_ARG() pNewHead->iPrimaryViewID + 1, VIEW_3DSKY );
	...
	else
	{
		...
		if ( pEnteredView )
		{
			...
			PixelVisibility_ShiftVisibilityViews( SS_SLOT_ARG() pEnteredView->iPrimaryViewID, VIEW_MAIN );
			PixelVisibility_ShiftVisibilityViews( SS_SLOT_ARG() pEnteredView->iPrimaryViewID + 1, VIEW_3DSKY );
		}
	}
...
{	  
	...
	{
#if ENGINE_NEW
		// state.m_bEnable = true;  // false and commented out in enhanced source
		state.m_CompareFunc = SHADER_STENCILFUNC_EQUAL;
		state.m_PassOp = SHADER_STENCILOP_KEEP;
		// state.m_FailOp = SHADER_STENCILOP_KEEP;
		// state.m_ZFailOp = SHADER_STENCILOP_KEEP;
