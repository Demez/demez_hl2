...
#define REORIENTATION_RATE cl_portal_reorient_rate.GetFloat()
#define REORIENTATION_ACCELERATION_RATE cl_portal_reorient_accel_rate.GetFloat()
...
#if ENGINE_NEW
#define RESET_EYE_ANGLES() m_iv_angEyeAngles.Reset(gpGlobals->curtime)
#else
#define RESET_EYE_ANGLES() m_iv_angEyeAngles.Reset()
#endif

// DEMEZ PORTAL: speed this up for vr?
...
// Purpose: 
//-----------------------------------------------------------------------------
int C_Portal_Player::DrawModel( int flags RENDER_INSTANCE_INPUT )
{
	if ( !m_bReadyToDraw )
		return 0;
...
bool C_Portal_Player::DetectAndHandlePortalTeleportation( void )
{
	if( m_bPortalledMessagePending )
	{
		...
		//if( pOldPortal )
		{
			...
			RESET_EYE_ANGLES(); //copies from m_angEyeAngles
...
{
	if ( m_bInVR )
		return CVRBasePlayerShared::GetAutoaimVector( flDelta );
...
void C_Portal_Player::CalcViewModelView( const Vector& eyeOrigin, const QAngle& eyeAngles)
{
	...
	for ( int i = 0; i < MAX_VIEWMODELS; i++ )
	{
		...
		if ( vm->GetOwningWeapon() && V_strcmp(vm->GetOwningWeapon()->GetClassname(), "weapon_portalgun") == 0 )
		{
