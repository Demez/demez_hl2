...
// move this to vr_controller.cpp when it works
IPhysicsMotionController* VR_GetPhysMotionControllerBase( IPhysicsObject *pSource, CVRBasePlayer* vrPlayer, CVRController* vrController )
{
	if ( !vrController )
		return NULL;

	CBaseEntity* pHeld = vrController->GetGrabbedObject();

	if ( !pHeld )
		return NULL;

	if( pSource->GetGameData() == (void*)pHeld )
	{
		...
		for ( int i = 0; i < count; i++ )
		{
			if ( pList[i] == pSource )
			{
				return vrController->m_pController;
			}
		}
	}

	// uh
	if ( vrController->GetGrabbedObject()->VPhysicsGetObject() == pSource )
		return vrController->m_pController;

	return NULL;
}

IPhysicsMotionController* VR_GetPhysMotionController( IPhysicsObject *pSource, CVRBasePlayer* vrPlayer )
{
	if ( !vrPlayer->m_bInVR )
		return NULL;

	IPhysicsMotionController* controller = VR_GetPhysMotionControllerBase( pSource, vrPlayer, vrPlayer->GetRightHand() );
	if ( !controller )
		controller = VR_GetPhysMotionControllerBase( pSource, vrPlayer, vrPlayer->GetLeftHand() );

	return controller;
}

void VR_GetSavedParamsForCarriedPhysObject( CVRController* controller, IPhysicsObject *pObject, float *pSavedMassOut, float *pSavedRotationalDampingOut )
{
	CBaseEntity *pHeld = controller->m_pGrabbedObject;
	if( pHeld )
	{
		if( pObject->GetGameData() == (void*)pHeld )
		{
			...
			for ( int i = 0; i < count; i++ )
			{
				...
				{
					if( pSavedMassOut )
						*pSavedMassOut = controller->m_savedMass;

					if( pSavedRotationalDampingOut )
						*pSavedRotationalDampingOut = controller->m_savedRotDamping;
...
static void FullSyncPhysicsObject( IPhysicsObject *pSource, IPhysicsObject *pDest, const VMatrix *pTransform, bool bTeleport )
{
	...
	if( pSource->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
	{
		...
		if( pHoldingPlayer )
		{
			...
			CVRBasePlayer* vrPlayer = (CVRBasePlayer*)pHoldingPlayer;
			//bIsObjectHeld = VR_GetPhysMotionController( pSource, vrPlayer );

			// the CVRController class has its own way of holding stuff, no CGrabController used
			if ( !vrPlayer->m_bInVR )
			{
				if ( !bIsObjectHeld )
					pGrabController = GetGrabControllerForPlayer( pHoldingPlayer );

				if ( !bIsObjectHeld && !pGrabController )
					pGrabController = GetGrabControllerForPhysCannon( pHoldingPlayer->GetActiveWeapon() );

				if ( !bIsObjectHeld )
					bIsObjectHeld = pGrabController;

				AssertMsg( bIsObjectHeld, "Physics object is held, but we can't find the holding controller." );
			}

			/*if ( vrPlayer->m_bInVR )
			{
				VR_GetSavedParamsForCarriedPhysObject();
			}
			else*/
			{
