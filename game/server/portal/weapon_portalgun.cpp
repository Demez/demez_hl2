...
float CWeaponPortalgun::FirePortal( bool bPortal2, Vector *pVector /*= 0*/, bool bTest /*= false*/ )
{
	...
	if( bPlayer )
	{
		...
		if ( !bTest && pPlayer && !pPlayer->m_bInVR )
		{
		...
		AngleVectors( pPlayer->GetWeaponShootAng(), &forward, &right, &up );
		// pPlayer->EyeVectors( &vDirection, NULL, NULL );
		vDirection = pPlayer->GetWeaponShootDir();
		...
		if ( pPlayerPortal && !pPlayer->m_bInVR )
		{
		...
		// VR: will need to adjust this, ideally it should be placed at the center of the controller
		// maybe tweaked a little to fit closer to the player direction
		if ( pPlayerPortal )
		{
