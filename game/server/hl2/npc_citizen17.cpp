...
int CNPC_Citizen::TranslateSchedule( int scheduleType ) 
{
	...
	case SCHED_RANGE_ATTACK1:
		...
		if ( !IsMortar( GetEnemy() ) && GetActiveWeapon() && FClassnameIs( GetActiveWeapon(), "weapon_rpg" ) )
		{
			...
			{
				CBasePlayer *pPlayer = UTIL_GetNearestPlayerPreferVisible(GetEnemy());
	...
	if ( IsAmmoResupplier() && bTargetIsPlayer )
	{
		if ( m_flPlayerGiveAmmoTime <= gpGlobals->curtime )
		{
			...
			else
			{
				...
				int iMax = GetAmmoDef()->AmmoMaxCarry(iAmmoType, (CBaseCombatCharacter *)pTarget);
...
{
	TossHealthKit( UTIL_GetNearestPlayerPreferVisible(this), Vector(48.0f, 0.0f, 0.0f)  );
}
