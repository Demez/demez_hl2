...
void CNPC_Vortigaunt::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_START_HEAL_GLOW )
	{
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_STOP_HEAL_GLOW )
	{
		EndHandGlow();
		return;
	}
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_START_HURT_GLOW )
	{
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_STOP_HURT_GLOW )
	{
		EndHandGlow();
		return;
	}
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_START_DISPEL )
	{
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_ACCEL_DISPEL )
	{
	...
	// Kaboom!
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_DISPEL )
	{
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_HEAL_PAUSE )
	{
		StartHealing();
		return;
	}

	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_ZAP_POWERUP )
	{
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_ZAP_SHOOT )
	{
		ClearBeams();
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_ZAP_DONE )
	{
		ClearBeams();
		return;
	}

	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_HEAL_STARTGLOW )
	{
		// Make hands glow
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_HEAL_STARTSOUND )
	{
		CPASAttenuationFilter filter( this );
	...
	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_SWING_SOUND )
	{
		EmitSound( "NPC_Vortigaunt.Swing" );	
		return;
	}

	if ( GetAnimEvent(pEvent) == AE_VORTIGAUNT_SHOOT_SOUNDSTART )
	{
		if ( m_fGlowChangeTime > gpGlobals->curtime )
			return;
	...
	if ( GetAnimEvent(pEvent) == AE_NPC_LEFTFOOT )
	{
		EmitSound( "NPC_Vortigaunt.FootstepLeft", pEvent->eventtime );
		return;
	}

	if ( GetAnimEvent(pEvent) == AE_NPC_RIGHTFOOT )
	{
