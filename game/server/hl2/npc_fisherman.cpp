...
{
	if ( GetAnimEvent(pEvent) == NPC_EVENT_LEFTFOOT )
	{
		EmitSound( "NPC_Fisherman.FootstepLeft", pEvent->eventtime );
	}
	else if ( GetAnimEvent(pEvent) == NPC_EVENT_RIGHTFOOT )
	{
		EmitSound( "NPC_Fisherman.FootstepRight", pEvent->eventtime );
	}
	else if ( GetAnimEvent(pEvent) == AE_FISHERMAN_HAT_UP )
	{
		if (m_iHatLayer != -1)
		{
	...
	else if ( GetAnimEvent(pEvent) == AE_FISHERMAN_HAT_DOWN )
	{
		if (m_iHatLayer != -1)
		{
			RemoveLayer( m_iHatLayer, 0.2, 0.2 );
	...
	else if ( GetAnimEvent(pEvent) == AE_FISHERMAN_HAT_ON )
	{
		m_iHatLayer = AddGesture( (Activity)m_iHatState, false );
	}
	else if ( GetAnimEvent(pEvent) == AE_FISHERMAN_HAT_OFF )
	{
		if (m_iHatLayer != -1)
		{
			RemoveLayer( m_iHatLayer, 0.2, 0.2 );
