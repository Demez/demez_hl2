...
{
	if( GetAnimEvent(pEvent) == AE_ACTBUSY_WEAPON_FIRE_ON )
	{
		m_bAutoFireWeapon = true;
		return;
	}
	else if( GetAnimEvent(pEvent) == AE_ACTBUSY_WEAPON_FIRE_OFF )
	{
		m_bAutoFireWeapon = false;
		return;
	}
...
int CAI_ActBusyBehavior::SelectScheduleWhileNotBusy( int iBase )
{
	...
	if ( m_bForceActBusy || m_flNextBusySearchTime < gpGlobals->curtime )
	{
		...
		if ( !pNode )
		{
			if( IsCombatActBusy() )
			{
				if ( m_hActBusyGoal->IsCombatActBusyTeleportAllowed() && m_iNumConsecutivePathFailures >= 2 && !UTIL_GetNearestPlayerPreferVisible(GetOuter())->FInViewCone(GetOuter()) )
				{
