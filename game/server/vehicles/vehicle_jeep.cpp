...
bool CPropJeep::CheckWater( void )
{
	...
	for ( int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel )
	{
		...
		{
			m_WaterData.m_bWheelInWater[iWheel] = ( Engine_UTIL_PointContents( m_WaterData.m_vecWheelContactPoints[iWheel], MASK_WATER ) ) ? true : false;
	...
	m_WaterData.m_bBodyInWater = ( Engine_UTIL_PointContents( vecEnginePoint, MASK_WATER ) ) ? true : false;
...
void CPropJeep::CheckWaterLevel( void )
{
	...
	if ( pEntity && pEntity->IsPlayer() )
	{
		...
		bool bEyes = ( Engine_UTIL_PointContents( vecAttachPoint, MASK_WATER ) ) ? true : false;
		if ( bEyes )
		{
		...
		GetAttachment( iAttachment, vecAttachPoint, vecAttachAngles );
		bool bFeet = ( Engine_UTIL_PointContents( vecAttachPoint, MASK_WATER ) ) ? true : false;
		if ( bFeet )
		{
