...
int CWeaponPortalBase::DrawModel( int flags RENDER_INSTANCE_INPUT )
{
	if ( !m_bReadyToDraw )
		return 0;

#if ENGINE_NEW
	if ( GetOwner() && (GetOwner() == C_BasePlayer::GetLocalPlayer()) && !g_pPortalRender->IsRenderingPortal() && !C_BasePlayer::GetLocalPlayer()->ShouldDrawLocalPlayer() )
		return 0;
#else
	...
	int iRetVal = BaseClass::DrawModel( flags RENDER_INSTANCE );
...
void CWeaponPortalBase::DrawCrosshair()
{
	...
	bool bOnTarget = false; // ( m_iState == WEAPON_IS_ONTARGET );
...
void CWeaponPortalBase::FireBullets( const FireBulletsInfo_t &infoBase )
{
	FireBulletsInfo_t info = infoBase;

#if ENGINE_NEW
	info.m_flPlayerDamage = GetPortalWpnData().m_iPlayerDamage;
#else
	info.m_iPlayerDamage = GetPortalWpnData().m_iPlayerDamage;
#endif
